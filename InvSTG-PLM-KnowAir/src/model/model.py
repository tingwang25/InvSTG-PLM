...
class NodeEmbedding(nn.Module):
    ...
    def setadj(self,adj_mx):
        ...
        if hasattr(self,'lap_eigvec'):
            device = self.lap_eigvec.device
            self.lap_eigvec = torch.as_tensor(eigvec, dtype=torch.float32, device=device)
        else :
            self.register_buffer('lap_eigvec', torch.as_tensor(eigvec, dtype=torch.float32))
...
class STALLM(nn.Module):
    ...
    def update_graph(self, adj_mx, dis_mx):
        device = self.ori_adj.device
        adj_tensor = adj_mx if torch.is_tensor(adj_mx) else torch.as_tensor(adj_mx, dtype=torch.float32, device=device)
        dis_tensor = dis_mx if torch.is_tensor(dis_mx) else torch.as_tensor(dis_mx, dtype=torch.float32, device=device)

        self.ori_adj = adj_tensor.to(device=device, dtype=torch.float32)
        self.dis_mx = dis_tensor.to(device=device, dtype=torch.float32)
        self.setadj(self.ori_adj, self.dis_mx)

        node_order, node_order_rev = topological_sort(self.ori_adj.detach().cpu().numpy())
        self.node_order = torch.as_tensor(node_order, dtype=torch.long, device=device)
        self.node_order_rev = torch.as_tensor(node_order_rev, dtype=torch.long, device=device)

        if self.use_node_embedding:
            self.node_embd_layer.setadj(self.ori_adj.detach().cpu().numpy())
