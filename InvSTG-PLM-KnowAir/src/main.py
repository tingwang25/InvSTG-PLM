...
from utils.metrics import (
    ...
    MAPE_torch_node,
    cal_metrics,
    cal_weighted_metrics,
    build_sudden_change_mask,
)
...
def TestEpoch(loader, model,  prompt_prefix, scaler, save=False, log_dir=None):
    ...
    with torch.no_grad():
        ...
        mae_conf, rmse_conf, mape_conf = None, None, None
        sudden_mae_pred, sudden_rmse_pred, sudden_ratio = None, None, None
        sudden_mae_conf, sudden_rmse_conf = None, None
        sudden_mask = None
        ...
        if args.task in ['all','prediction']:
            ...
            pred_targets = targets[:,-args.predict_len:]
            pred_predicts = predicts[:,-args.predict_len:]
            pred_predict_confs = predict_confs[:,-args.predict_len:]

            mae_pred, rmse_pred, mape_pred, _,_ = cal_metrics(predicts=pred_predicts,targets=pred_targets,eval_mask=eval_mask)
            mae_conf, rmse_conf, mape_conf, _,_ = cal_metrics(predicts=pred_predict_confs,targets=pred_targets,eval_mask=eval_mask)

            if args.eval_sudden_change:
                sudden_mask = build_sudden_change_mask(
                    targets=pred_targets,
                    eval_mask=eval_mask,
                    threshold_start=args.sudden_threshold_start,
                    threshold_change=args.sudden_threshold_change,
                )
                sudden_mae_pred, sudden_rmse_pred, sudden_ratio = cal_weighted_metrics(
                    predicts=pred_predicts,
                    targets=pred_targets,
                    weight_mask=sudden_mask,
                    reference_mask=eval_mask,
                )
                sudden_mae_conf, sudden_rmse_conf, _ = cal_weighted_metrics(
                    predicts=pred_predict_confs,
                    targets=pred_targets,
                    weight_mask=sudden_mask,
                    reference_mask=eval_mask,
                )
    ...
    if save:
        ...
        save_payload = {
            ...
            'mask': eval_masks.cpu().numpy(),
        }
        if sudden_mask is not None:
            save_payload['prediction_sudden_change_mask'] = sudden_mask.cpu().numpy()
        np.savez(os.path.join(log_dir,'test.npz'), **save_payload)

    sudden_metrics = {
        'mae_pred': sudden_mae_pred,
        'rmse_pred': sudden_rmse_pred,
        'ratio': sudden_ratio,
        'mae_conf': sudden_mae_conf,
        'rmse_conf': sudden_rmse_conf,
    }

    return mae_recon, rmse_recon, mape_recon, mae_pred, rmse_pred, mape_pred, mae_conf, rmse_conf, mape_conf, sudden_metrics
...
def Train(args, mylogger, model, prompt_prefix, scaler, train_loader, val_loader, test_loader, LOG_DIR,
    ...
    for epoch in range(max_epoch):
        model.update_graph(adj_mx_train, distance_mx_train)
        ...
        if epoch % test_epoch == 0:
            model.update_graph(adj_mx_test, distance_mx_test)

            mae_recon, rmse_recon, mape_recon, mae_pred, rmse_pred, mape_pred, mae_conf, rmse_conf, mape_conf, sudden_metrics = TestEpoch(test_loader,model,prompt_prefix,scaler=scaler)
            model.update_graph(adj_mx_train, distance_mx_train)
            ...
            if args.task in ['all','prediction']:
                ...
                if args.eval_sudden_change:
                    mylogger.info(
                        f"[Test][prediction][sudden_change] epoch {epoch} "
                        f"threshold_start:{args.sudden_threshold_start} "
                        f"threshold_change:{args.sudden_threshold_change} "
                        f"coverage:{sudden_metrics['ratio']} "
                        f"mae:{sudden_metrics['mae_pred']} rmse:{sudden_metrics['rmse_pred']} "
                        f"mae_conf:{sudden_metrics['mae_conf']} rmse_conf:{sudden_metrics['rmse_conf']}"
                    )
    ...
    mae_recon, rmse_recon, mape_recon, mae_pred, rmse_pred, mape_pred, mae_conf, rmse_conf, mape_conf, sudden_metrics = TestEpoch(test_loader,model,prompt_prefix,scaler,save=args.save_result, log_dir=LOG_DIR)
    ...
    if args.task in ['all','prediction']:
        ...
        if args.eval_sudden_change:
            mylogger.info(
                f"[Test][prediction][sudden_change] best model "
                f"threshold_start:{args.sudden_threshold_start} "
                f"threshold_change:{args.sudden_threshold_change} "
                f"coverage:{sudden_metrics['ratio']} "
                f"mae:{sudden_metrics['mae_pred']} rmse:{sudden_metrics['rmse_pred']} "
...
def main(args):
    ...
    train_loader, val_loader, test_loader, scaler, features, adj_mx_train, distance_mx_train, adj_mx_test, distance_mx_test = load_data_ood(
        ...
        knowair_target_col=args.knowair_target_col,
    )
