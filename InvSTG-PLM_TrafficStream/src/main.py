...
def TrainEpoch(loader, model, optim, conf_opt, loss_fn,  prompt_prefix, scaler, epoch, alpha, need_step: bool,
               mylogger=None, adj_mx=None, distance_mx=None):
    update_model_graph(model, adj_mx, distance_mx, args.device)
    if need_step:
        model.train()
        var_ratio = 1
        init_epoch = 5
...
def TestEpoch(loader, model,  prompt_prefix, scaler, save=False, adj_mx=None, distance_mx=None):
    update_model_graph(model, adj_mx, distance_mx, args.device)
...
def Train(args, mylogger, model, prompt_prefix, scaler, train_loader, val_loader, test_loader, LOG_DIR,
          adj_mx_train=None, distance_mx_train=None, adj_mx_test=None, distance_mx_test=None):
    patience_count = 0
    ...
    for epoch in range(max_epoch):
        ...
        train_loss, conf_loss = TrainEpoch(
            train_loader, model, optim, conf_opt, loss_fn, prompt_prefix, scaler, epoch, args.alpha,
            need_step=True, mylogger=mylogger, adj_mx=adj_mx_train, distance_mx=distance_mx_train
        )
        ...
        if epoch % val_epoch == 0:
            ...
            val_loss, val_conf_loss = TrainEpoch(
                val_loader, model, optim, conf_opt, loss_fn, prompt_prefix, scaler, epoch, args.alpha,
                need_step=False, mylogger=mylogger, adj_mx=adj_mx_train, distance_mx=distance_mx_train
            )
        ...
        if epoch % test_epoch == 0:
            ...
            mae_recon, rmse_recon, mape_recon, mae_pred, rmse_pred, mape_pred, mae_conf, rmse_conf, mape_conf = TestEpoch(
                test_loader, model, prompt_prefix, scaler=scaler, adj_mx=adj_mx_test, distance_mx=distance_mx_test
            )
    ...
    mae_recon, rmse_recon, mape_recon, mae_pred, rmse_pred, mape_pred, mae_conf, rmse_conf, mape_conf = TestEpoch(
        test_loader, model, prompt_prefix, scaler, save=args.save_result, adj_mx=adj_mx_test, distance_mx=distance_mx_test
    )
...
def update_model_graph(model, adj_mx, distance_mx, device):
    if adj_mx is None or distance_mx is None or not hasattr(model, 'setadj'):
        return

    adj_tensor = adj_mx if torch.is_tensor(adj_mx) else torch.as_tensor(adj_mx, dtype=torch.float32, device=device)
    dis_tensor = distance_mx if torch.is_tensor(distance_mx) else torch.as_tensor(distance_mx, dtype=torch.float32, device=device)
    model.ori_adj = adj_tensor
    model.setadj(adj_tensor, dis_tensor)
...
def main(args):
    ...
    train_loader, val_loader, test_loader, scaler, features, adj_mx_train, distance_mx_train, adj_mx_test, distance_mx_test = load_data_ood(
        ...
        years=args.years,
        checkyears=args.checkyears,
        tood=args.tood,
        sood=args.sood,
    )
    if adj_mx_train is not None and hasattr(adj_mx_train, 'shape'):
        args.num_nodes = int(adj_mx_train.shape[0])
    ...
    Train(
        args, mylogger, model, prompt_prefix, scaler, train_loader, val_loader, test_loader, LOG_DIR,
        adj_mx_train=adj_mx_train, distance_mx_train=distance_mx_train,
        adj_mx_test=adj_mx_test, distance_mx_test=distance_mx_test,
    )
